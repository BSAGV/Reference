...
configuration.draw_ellipses = 0;
configuration.draw_trunks = 1;
configuration.draw_intermediate_hypotheses = 0;
...
if configuration.small_map
 ...
 CreatMapLength(features);
% obtain observations
scan = sick(step,:);
observations = obs (step);

distances = zeros(features.n, features.n);
dist2 = zeros(observations.m, observations.m);

figure(1); clf;
draw_features (features, 'b');
draw_vehicle (ground, 'b');
draw_trajectory (vehicle.ground, 'b');
